...
namespace CoreRT
{
    public partial struct Decimal2
    {
        ...
        internal static uint DecDivMod1E9(ref Decimal2 value)
        {
            return DecCalc.DecDivMod1E9(ref value);
        }

        internal static void DecAddInt32(ref Decimal2 value, uint i)
        {
            DecCalc.DecAddInt32(ref value, i);
        }

        internal static void DecMul10(ref Decimal2 value)
        {
        ...
        /// </summary>
        private unsafe class DecCalc
        {
            // Constant representing the negative number that is the closest possible
            // Decimal value to -0m.
            private static readonly Decimal2 NearNegativeZero = -0.000000000000000000000000001m;

            // Constant representing the positive number that is the closest possible
            // Decimal value to +0m.
            private static readonly Decimal2 NearPositiveZero = +0.000000000000000000000000001m;
...
// No deivrem for unsiged uint
            ...
            ***********************************************************************/
            ...
            private static bool DecAddSub(ref Decimal2 d1, ref Decimal2 d2, bool bSign)
            {
                uint* rgulNum = stackalloc uint[6];
                uint ulPwr;
                int iScale;
                int iHiProd;
                int iCur;
            ...
            private static void SignFlip(ref Decimal2 value)
            {
            ...
            private static bool AlignedScale(ref Decimal2 value)
            {
                Split64 sdlTmp = new Split64();
            ...
            private static bool LongSub(ref Decimal2 value, ref int iHiProd, uint* rgulNum)
            {
            ...
            // Returns true if we overflowed
            private static bool AlignedAdd(ref Decimal2 value, ref Decimal2 d1, ref Decimal2 d2, bool bSign)
            {
                if (bSign)
                {
                    // Signs differ - subtract
            ...
            // is -d.
            //
            private static Decimal2 Abs(Decimal2 d)
            {
...
*
***********************************************************************/
            private static uint DecFixInt(ref Decimal2 input, ref Decimal2 result)
            {
                uint* tmpNum = stackalloc uint[3];
                uint remainder;
                uint power;
                int scale;
            ...
            //**********************************************************************
            internal static void VarCyFromDec(ref Decimal2 pdecIn, out long pcyOut)
            {
                if (!Decimal2.IsValid(pdecIn.uflags))
            ...
            //**********************************************************************
            internal static int VarDecCmp(ref Decimal2 pdecL, ref Decimal2 pdecR)
            {
                int signLeft = 0;
                int signRight = 0;
                ...
                if (signLeft == signRight)
                {
                    ...
                    Decimal2 decLAndResult = pdecL; // Copy the left and pass that to AddSub because it gets mutated.
            ...
            //**********************************************************************
            internal static void VarDecMul(ref Decimal2 pdecL, ref Decimal2 pdecR, out Decimal2 pdecRes)
            {
                Split64 sdlTmp = new Split64();
                Split64 sdlTmp2 = new Split64();
                Split64 sdlTmp3 = new Split64();
                ...
                uint ulRemLo;
                uint ulRemHi;
                uint* rgulProd = stackalloc uint[6];

                pdecRes = new Decimal2();
            ...
            //**********************************************************************
            internal static void VarDecFromR4(float input, out Decimal2 pdecOut)
            {
            ...
            //**********************************************************************
            internal static void VarDecFromR8(double input, out Decimal2 pdecOut)
            {
                int iExp;    // number of bits to left of binary point
            ...
            //**********************************************************************
            internal static float VarR4FromDec(ref Decimal2 pdecIn)
            {
            ...
            //**********************************************************************
            internal static double VarR8FromDec(ref Decimal2 pdecIn)
            {
            ...
            // of the operation
            internal static void VarDecAdd(ref Decimal2 d1, ref Decimal2 d2)
            {
            ...
            // of the operation.
            internal static void VarDecSub(ref Decimal2 d1, ref Decimal2 d2)
            {
            ...
            // of the operation.
            internal static void VarDecDiv(ref Decimal2 d1, ref Decimal2 d2)
            {
                uint* rgulQuo = stackalloc uint[3];
                uint* rgulQuoSave = stackalloc uint[3];
                uint* rgulRem = stackalloc uint[4];
                uint* rgulDivisor = stackalloc uint[3];
                uint ulPwr;
                uint ulTmp;
                uint ulTmp1;
                Split64 sdlTmp = new Split64();
            ...
            //**********************************************************************
            internal static void VarDecInt(ref Decimal2 d)
            {
                Decimal2 result = new Decimal2();
            ...
            //**********************************************************************
            internal static void VarDecFix(ref Decimal2 d)
            {
                Decimal2 result = new Decimal2();
            ...
            //**********************************************************************
            internal static void VarDecRound(ref Decimal2 input, int decimals, ref Decimal2 result)
            {
                uint* tmpNum = stackalloc uint[3];
                uint remainder;
                uint sticky;
            ...
            //**********************************************************************
            internal static Decimal2 VarDecMod(ref Decimal2 d1, ref Decimal2 d2)
            {
                ...
                Decimal2 dividedResult = Truncate(d1 / d2);
                Decimal2 multipliedResult = dividedResult * d2;
                Decimal2 result = d1 - multipliedResult;
            ...
            private static void InternalAddUInt32RawUnchecked(ref Decimal2 value, UInt32 i)
            {
                UInt32 v;
                UInt32 sum;
                v = value.ulo;
            ...
            private static UInt32 InternalDivRemUInt32(ref Decimal2 value, UInt32 divisor)
            {
                UInt32 remainder = 0;
                UInt64 n;
                if (value.uhi != 0)
            ...
            // away from zero
            internal static void InternalRoundFromZero(ref Decimal2 d, int decimalCount)
            {
                Int32 scale = d.Scale;
            ...
            private static void DecShiftLeft(ref Decimal2 value)
            {
            ...
            private static void DecAdd(ref Decimal2 value, Decimal2 d)
            {
