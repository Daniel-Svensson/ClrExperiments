    ...
    public partial struct Decimal
    {
        ...
        public struct DecCalc_Main
        {
            ...
            private static uint Div128By96(ref Buf16 bufNum, ref Buf12 bufDen)
            {
                ...
                // Compute full remainder, rem = dividend - (quo * divisor).
                //
                ulong prod1 = MathBigMul(quo, bufDen.U0); // quo * lo divisor
                ulong prod2 = MathBigMul(quo, bufDen.U1); // quo * mid divisor
                prod2 += prod1 >> 32;
            ...
            private static uint IncreaseScale_Main(ref Buf12 bufNum, uint power)
            {
                ...
                bufNum.U0 = (uint)tmp;
                tmp >>= 32;
                tmp += MathBigMul(bufNum.U1, power);
                bufNum.U1 = (uint)tmp;
                tmp >>= 32;
                tmp += MathBigMul(bufNum.U2, power);
                bufNum.U2 = (uint)tmp;
                return (uint)(tmp >> 32);
            }
            ...
            internal static long VarCyFromDec(ref DecCalc_Main pdecIn)
            {
                ...
                if (scale < 0)
                {
                    ...
                    ulong high = MathBigMul(pwr, pdecIn.Mid);
                    if (high > uint.MaxValue)
                        goto ThrowOverflow;
                ...
                do
                {
                    if (scale < 0)
                    {
                        ...
                        do
                        {
                            ...
                            scale += iCurScale;
                            ulong tmp = MathBigMul(bufQuo.U0, power);
                            bufQuo.U0 = (uint)tmp;
                            tmp >>= 32;
